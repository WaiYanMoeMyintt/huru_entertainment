...
const  PrimarySeries  = () => {
  ...
  function getNumbers(){
      ...
      const arr = [];
      for (let i = 1; i < num; i++){
        const primary_api = `https://api.themoviedb.org/3/discover/tv?api_key=1b7c076a0e4849aeefd1f3c429c99a36&include_adult=false&include_video=false&language=en-US&page=${i}&sort_by=popularity.desc`;
  ...
  return (
    <div className="primary_movies_list">
      ...
      {primary &&
        primary.map((items) => (
          <Link to = {`/series/${items.id}/${items.original_title}`} key={items.id} className="primary_movies">
